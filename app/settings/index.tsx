...
// === Secao Dev: ferramentas de desenvolvimento (__DEV__-only) ===
//
// R-HOME-4c: o link "Storybook de componentes" saiu da Tela Hoje (era um
// bloco __DEV__ no rodape da home) e passou para aqui. A rota /_components
// segue existindo (util no fluxo Gauntlet e em useUltimaRota); so o ponto
// de acesso mudou. Renderizada apenas sob __DEV__ (dead-code em release),
// no fim da lista de settings. Nao poluir a home nem aparecer em producao.

function SecaoDev() {
  const router = useRouter();
  return (
    <SecaoLista titulo="Desenvolvimento" accessibilityLabel="secao dev">
      <LinkSubTela
        titulo="Storybook de componentes"
        subtitulo="Galeria dos componentes da UI para inspeção visual."
        onPress={() => router.push('/_components')}
