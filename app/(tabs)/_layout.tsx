...
//   - 5 abas fixas (CONTRACT secao 1.1):
//       1. /(tabs)/           Hoje      (M02 - tela real)
//       2. /(tabs)/memoria    Memorias  (M11 - stub redirect)
...
//
// Como as sprints donas (M10/M11/M14/M15 etc) ainda nao rodaram,
...
// (ciclo.tsx etc) tambem renderiza <Redirect> ate sua sprint dona.
...
export default function TabsLayout() {
  ...
  return (
    ...
    >
      {/* Aba fixa 1: Hoje (M02 ja entregou app/(tabs)/index.tsx). */}
      <Tabs.Screen name="index" options={{ title: 'Hoje' }} />

      {/* Aba fixa 2: Memorias (stub redirect ate M11). */}
      <Tabs.Screen name="memoria" options={{ title: 'Memorias' }} />

      {/* Aba fixa 3: Humor (stub redirect ate M10). */}
      <Tabs.Screen name="humor" options={{ title: 'Humor' }} />

      {/* Aba fixa 4: Financas (stub redirect ate M14). */}
      <Tabs.Screen name="financas" options={{ title: 'Financas' }} />

      {/* Aba fixa 5: Settings (stub redirect ate M15). */}
      <Tabs.Screen name="settings" options={{ title: 'Settings' }} />

      {/* Stub generico sempre acessivel via deep link. Esconde da
          bottom bar via href:null para nao virar 6a aba fixa. */}
      <Tabs.Screen
        name="em-construcao"
