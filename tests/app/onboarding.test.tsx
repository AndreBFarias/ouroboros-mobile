// Smoke do onboarding 6 frames (J1 + R-HOME-4e seed opcional).
// Cobre:
...
//  - Frame 0 -> 1 -> 2 -> 3 (Permissoes) -> 4 (Seed opcional) -> 5 (Tudo pronto)
...
//  - Frame 4 (seed R-HOME-4e): "Pular" avanca sem gravar para Frame 5
//  - Frame 5: Comecar marca onboarding concluido e router.replace
//  - Frame 5: resumo "N permissoes concedidas" reflete contagem real
...
// Helper R-HOME-4e: no Frame 4 (seed opcional), "Pular" sem gravar
// avanca para o Frame 5 (Tudo pronto), preservando o fecho testado.
function pularSeed(api: { getByLabelText: (l: string) => unknown }) {
  fireEvent.press(api.getByLabelText('pular seed onboarding') as never);
}

describe('Onboarding J1 + R-HOME-4e - 6 frames com permissoes e seed', () => {
  ...
  it('Frame 3 "Continuar": chama request* para toggles ON, persiste em store, avanca para Frame 4 (seed) e Pular leva ao Frame 5', async () => {
    const api = renderTela();
    avancarAtePasta(api);
    fireEvent.press(api.getByLabelText('usar sugestao ouroboros'));
    await waitFor(() => {
    ...
    // R-HOME-4e: Frame 3 Continuar agora leva ao Frame 4 (seed opcional);
    // Pular sem gravar avanca para o Frame 5 (Tudo pronto).
    await waitFor(() => {
      expect(api.getByLabelText('pular seed onboarding')).toBeTruthy();
    });
    pularSeed(api);
    await waitFor(() => {
  ...
  it('Frame 3: requestCamera retorna false persiste false', async () => {
    ...
    fireEvent.press(api.getByText('Continuar'));
    // R-HOME-4e: passa pelo Frame 4 (seed); Pular avanca ao Frame 5.
    await waitFor(() => {
      expect(api.getByLabelText('pular seed onboarding')).toBeTruthy();
    });
    pularSeed(api);
  ...
  it('Frame 4: resumo mostra contagem singular/plural correto', async () => {
    ...
    fireEvent.press(api.getByText('Continuar'));
    // R-HOME-4e: Pular o Frame 4 (seed) para chegar ao Frame 5 (resumo).
    await waitFor(() => {
      expect(api.getByLabelText('pular seed onboarding')).toBeTruthy();
    });
    pularSeed(api);
  ...
  it('Frame 4: Comecar marca concluido e redireciona', async () => {
    ...
    fireEvent.press(api.getByText('Continuar'));
    // R-HOME-4e: Pular o Frame 4 (seed) para chegar ao Frame 5 (fecho).
    await waitFor(() => {
      expect(api.getByLabelText('pular seed onboarding')).toBeTruthy();
    });
    pularSeed(api);
