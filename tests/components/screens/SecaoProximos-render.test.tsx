// Teste de integracao RTL para SecaoProximos (R-HOME-2, atualizado em
// R-HOME-4a). Mocka useProximos com fixture deterministica e valida que:
...
//  - R-HOME-4a self-hide: lista vazia / loading / error retornam null
//    (a secao inteira some -- nada de "Nada nas próximas horas." nem
//    "Carregando..." no feed adaptativo).
//
...
test('R-HOME-4a empty state: lista vazia -> self-hide (secao ausente)', () => {
  mockDados = {
    itens: [],
    loading: false,
    error: null,
  ...
  const { queryByText } = render(<SecaoProximos />);
  // Self-hide: nem titulo nem caixa "Nada ..." aparecem.
  expect(queryByText('Próximos')).toBeNull();
  expect(queryByText('Nada nas próximas horas.')).toBeNull();
});

test('R-HOME-4a loading state: -> self-hide (sem "Carregando...")', () => {
  mockDados = {
    itens: [],
    loading: true,
    error: null,
...
test('R-HOME-4a error state: -> self-hide (secao ausente, sem itens)', () => {
  mockDados = {
    itens: [],
    loading: false,
    error: 'falha mock',
  ...
  const { queryByText } = render(<SecaoProximos />);
  // Comportamento: error => null (secao inteira some).
  expect(queryByText('Próximos')).toBeNull();
  expect(queryByText('Nada nas próximas horas.')).toBeNull();
  expect(queryByText('Carregando...')).toBeNull();
});
