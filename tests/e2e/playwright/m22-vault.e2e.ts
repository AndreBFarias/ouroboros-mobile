...
export default async function caseM22(
  ...
  try {
    ...
    const path =
      'docs/validacao-gauntlet-2026-05-03/screenshots/M22/A-vault-setado.png';
    ...
    if (estado.vaultRoot !== 'web://mock-vault/Ouroboros') {
      return {
        ...
        aspecto,
        status: 'FAIL',
        detalhe: `vaultRoot=${estado.vaultRoot}`,
        screenshots,
      };
    }
    return {
      ...
      aspecto,
      status: 'PASS',
      detalhe: 'vaultRoot=web://mock-vault/Ouroboros',
      screenshots,
    };
  } catch (err) {
    return {
