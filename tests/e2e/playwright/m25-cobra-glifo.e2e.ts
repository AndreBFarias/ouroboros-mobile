...
export default async function caseM25(
  ...
  try {
    ...
    const r = await page.evaluate(() => {
      ...
      const transforms = Array.from(gs)
    ...
    const path =
      'docs/validacao-gauntlet-2026-05-03/screenshots/M25/A-loader-renderizado.png';
    ...
    if (!r.ok || (r.qtdG ?? 0) < 4) {
      return {
        ...
        aspecto,
        status: 'FAIL',
        detalhe: `glifo invalido: ${JSON.stringify(r)}`,
        screenshots,
      };
    }
    return {
      ...
      aspecto,
      status: 'PASS',
      detalhe: `${r.qtdG} grupos, transform string ok`,
      screenshots,
    };
  } catch (err) {
    return {
