...
export default async function caseM20(
  ...
  try {
    ...
    const path =
      'docs/validacao-gauntlet-2026-05-03/screenshots/M20/A-toggle-settings.png';
    ...
    if (!tem) {
      return {
        ...
        aspecto,
        status: 'FAIL',
        detalhe: 'toggle Widget ausente em Settings',
        screenshots,
      };
    }
    return {
      ...
      status: 'INCONCLUSIVO',
      detalhe:
        'toggle web presente; widget Android nativo precisa Nivel B (emulador)',
      screenshots,
    };
  } catch (err) {
    return {
