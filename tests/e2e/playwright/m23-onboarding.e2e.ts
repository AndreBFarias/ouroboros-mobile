...
export default async function caseM23(page: PlaywrightPageLike): Promise<ResultadoE2E> {
  const sprint = 'M23';
  ...
  try {
    ...
    const ok = await page.evaluate(() =>
      document.body.innerText.includes('Como voc') && document.body.innerText.includes('chama')
    );

    const path = 'docs/validacao-gauntlet-2026-05-03/screenshots/M23/A-frame1-nome.png';
    await page.screenshot({ path });
    screenshots.push(path);

    if (!ok) {
      return { sprint, aspecto, status: 'FAIL', detalhe: 'titulo Frame 1 ausente', screenshots };
    }
    return { sprint, aspecto, status: 'PASS', detalhe: 'Frame 1 renderiza', screenshots };
  } catch (err) {
