// E2E R-HOME-1 -- valida a Tela Hoje (ADR-0026, D1=C) ja no formato da
// home viva (R-HOME-4a/4c). Reconciliado em R-HOME-4d: Proximos/To-do
// agora se AUTO-OCULTAM quando sem substancia e o BotaoRecap standalone
// foi removido na 4c -- entao a primeira fold num dia sem dados garante
// apenas saudacao + Reflexao + card "Voces"/"Voce" (ancora garantida do
// feed). Cobre:
//   A) Primeira fold renderiza cabecalho com saudacao + Reflexao + card
//      Voces (garantido), SEM Status do casal, SEM Humor do dia, SEM
//      Esta jornada (removidos em D1=C).
  ...
  try {
    ...
    const layout = await page.evaluate(() => {
      ...
      return {
        ...
        // Ancora garantida do feed (R-HOME-4a): o card de estado do casal
        // renderiza sempre. Titulo "Você" no modo sozinho (default do
        // seed sem nomeB) e "Vocês" no modo casal.
        temVoces: tem('Vocês') || tem('Você'),
        // Proximos/To-do agora se auto-ocultam sem substancia (R-HOME-4a)
        // e o BotaoRecap standalone saiu na 4c -- por isso deixaram de ser
        // invariantes de sempre-presenca. Nao afirmamos presenca nem
        // ausencia deles aqui (dependem do conteudo do seed).
    ...
    if (!layout.temSaudacao || !layout.temReflexao || !layout.temVoces) {
      return {
        sprint,
        aspecto,
        status: 'FAIL',
        detalhe: `invariantes da home viva ausentes (saudacao + Reflexao + card Voces): ${JSON.stringify(layout)}`,
        screenshots,
      };
    }
    ...
    return {
      ...
      aspecto,
      status: 'PASS',
      detalhe: `Tela Hoje (home viva) OK: saudacao + Reflexao + card Voces garantido, sem Status casal/Humor/Jornada + persist otimista de tarefa + navegacao Reflexao -> /diario-emocional?modo=reflexao`,
      screenshots,
    };
  } catch (err) {
    return {
