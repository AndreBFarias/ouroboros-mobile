...
describe('destinoRelembranca (R-HOME-4d)', () => {
  it('reflexao -> /diario-emocional com slug', () => {
    expect(
      destinoRelembranca({
        origem: 'reflexao',
        id: 'diario_reflexao:2025-07-10:pessoa_a',
      })
    ).toEqual({
      pathname: '/diario-emocional',
      params: { slug: 'diario_reflexao:2025-07-10:pessoa_a' },
    });
  });

  it('conquista -> /diario-emocional com slug', () => {
    expect(
      destinoRelembranca({
        origem: 'conquista',
        id: 'diario_vitoria:2025-07-10:pessoa_b',
      })
    ).toEqual({
      pathname: '/diario-emocional',
      params: { slug: 'diario_vitoria:2025-07-10:pessoa_b' },
    });
  });

  it('humor -> /humor (historico, sem params)', () => {
    expect(
      destinoRelembranca({ origem: 'humor', id: 'humor:2025-06-01' })
    ).toEqual({ pathname: '/humor' });
  });
});
