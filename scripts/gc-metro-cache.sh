...
#   textuais a worktrees ja removidos (ex: worktrees/agent-XXXX).
...
#   - Para cada entrada, faz grep -l por strings "worktrees/<id>".
...
#      <linha em branco>
#      worktree /path/to/worktrees/agent-XXX
#      ...
ACTIVE_WORKTREES=()
MAIN_REPO=""
...
for entry in "$CACHE_ROOT"/*; do
  ...
  # Filtra por padrao generico "worktrees/" -- se nao casar
  ...
  while IFS= read -r refline; do
    ...
    grep -roahE --include="*" --binary-files=text \
      'worktrees/[a-zA-Z0-9_-]+' "$entry" 2>/dev/null \
      | sed -E 's|.*worktrees/([a-zA-Z0-9_-]+).*|\1|' \
      | sort -u
  )
