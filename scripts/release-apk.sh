...
if [[ -n "${EXPO_ACCOUNT:-}" ]]; then
  echo "    Acompanhe em: https://expo.dev/accounts/${EXPO_ACCOUNT}/projects/ouroboros-mobile/builds/$BUILD_ID"
else
  echo "    Acompanhe o build no painel do EAS (expo.dev). Build ID: $BUILD_ID"
fi
