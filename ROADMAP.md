...
| `[todo]` | M-GAUNTLET-LEAK-CHECK | Script CI que roda `expo export --platform android` + grep `__gauntlet` confirmando dead-code | — | — | 1-2h | `M-GAUNTLET-LEAK-CHECK-spec.md` |
| `[todo]` | M-GAUNTLET-SEED-V2 | Fixtures realistas em `seedDeterministico` (`seedHumores` 30d, `seedDiarios` 3, `seedEventos` 7) + API `seedComDados(fixture)` | — | — | 3-4h | `M-GAUNTLET-SEED-V2-spec.md` |
