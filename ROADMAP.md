...
| R-ROT-2 | Rotinas escopo expandido (categorias) | docs+feature | P3 | `7078765` | `[ok]` (3N.7) ROTINA_CATEGORIAS (medicacao/saude_fisica/habito/outro) + chips em /rotinas/novo + 3 templates pre-preenchidos + empty state ampliado + FEATURES-CANONICAS §4.5; +8 testes (285/2712); 3 screenshots CDP em /tmp/ |
...
| R-MICROFONE-USE-AFTER-UNMOUNT | mountedRef em MicrofoneButton stopAndUnloadAsync().then (mesma classe do hunt-3) | fix | P2 | `fa094fe` | `[ok]` (3K.1) 18 ocorrencias mountedRef + 3 testes unmount cenarios |
| R-INFRA-WORKTREE-ENV-SYMLINK | Bootstrap defensivo de symlinks em smoke.sh (worktree fresh sem hook trigger) | infra+DX | P2 | `6b6d013` | `[ok]` (3K.2) +9 linhas smoke.sh + 11 linhas CONTEXTO; validacao empirica worktree fresh OK |
| R-INFRA-GOOGLE-AUTH-FLOW-TEST-FIX | jest.doMock no beforeEach (require.cache poluído de env.json) | fix | P3 | `b2e8d6e` | `[ok-resolvido-em-hunt-5]` (3K.3) 10/10 verde isolado antes e depois; sprint registra durabilidade via comentario +12L |
| R-INTEGRACOES-CANCELADO-PATTERN | Refator let cancelado -> mountedRef em IntegracoesScreen (consistência hunt-3) | refactor | P3 | `ac2d42b` | `[ok]` (3K.4) let cancelado 0 matches; mountedRef 8 occorrencias; +1 teste use-after-unmount |
| R-BUNDLE-SIZE-AUDIT | Auditoria do bundle Hermes antes do M41 (8.5 -> 10.23 MB em 17 dias; +44.5%) | audit | P2 | `0f8b604` workdir | `[ok]` (3M.1) relatorio + breakdown em docs/auditoria-bundle-2026-05-21/; top 1 lucide regressao tree-shake +1305KB; top 2 react-native-calendars+lodash+moment+recyclerlistview +1252KB |
| R-BUNDLE-LUCIDE-RESHIM | Fix app/index.tsx:29 bypass shim icons + ESLint no-restricted-imports lucide-react-native | fix | P1 | worktree-only (consolidado) | `[ok]` (3M.2) **ganho -1.67MB (-17.14%) — 2.7x acima do esperado**; bundle 10.23->8.48MB Hermes; ESLint guard com 3 testes empiricos OK |
...
| R-SCHEMA-TREINO-SESSAO-ROTINA-SLUG | Campo `rotina_slug` opcional em TreinoSessaoSchema (parte 1 de D) | schema migration | P2 | `9148823` | `[ok]` (3N.5) campo opcional + writer + executor pass slug + reader filter; +7 testes; backward-compat verificada (sessoes legado sem campo); smoke 282/2661 verde 3 runs |
| R-ROT-1-D-INTELIGENCIA-TREINO | Inteligencia temporal sobre treinos (parte 2 de D, consome rotina_slug) | feature | P2 | `5d0184c` | `[ok]` (3N.8) helper inteligenciaTemporal + banner SugestaoAlarmeRotina + schema silenciar_sugestao_ate + writer silenciarSugestaoRotina + integracao /rotinas/[slug]; +32 testes (287/2744); 2 screenshots playwright |
...
| R-INT-3-HC-BACKGROUND-DEFINETASK-SCOPE | fix | P2 | `[ok e26e632]` defineTask no escopo global do modulo (roda no import via _layout); tsc 0, jest 3/3. Confirmar comportamento na validacao live | achado sessao SDK56/autopull-bg |

#### 3P.AB — Onda contingentes pos-SDK56 (2026-05-26)

Dono pediu execucao dos 9 contingentes. 4 via agentes em worktree, 2 fixes orquestrador,
1 descope, 1 analise, 1 achado infra. Smoke 323->327 / 3113 (+40). Build pendente.

| ID | Tipo | P | Status |
|---|---|---|---|
| R-INT-3-HC-DEDUP | fix | P2 | `[ok 2b2e265]` clientRecordId deterministico + factories *WithId (AAR decompilado); +7 testes |
| R-INT-3-HC-SYNC-PAINEL | feature | P3 | `[ok eeb3909]` painel status+manual+telemetria em /settings/integracoes; consolida SETTINGS-STATUS+UI-MANUAL+TELEMETRIA; helper haRelativo |
| R-RECAP-7-SHARE-FORMATO-QUADRADO | feature | P3 | `[ok c914106]` share 1080x1080 (feed) + 1080x1920 (stories); escolha de formato |
| R-INT-3-LOGGER-CONDICIONAL | fix | P3 | `[ok 975a6d8]` devLog gate __DEV__; 14 logs fora do logcat release |
| R-INT-4-YOUTUBE-MUSIC-HISTORY | feature | P3 | `[descopado v1.1 4c6ee54]` API v3 sem watch history; YT Music so via eng. reversa (viola ADR-0007) |
| R-SEC-7-PROGUARD-HC-EXTENDED | infra | P3 | `[ok-analise e26e632]` regra `client.** {*;}` ja recursiva cobre request/units; redundante, sem codigo |
| (infra) bootstrap-worktree expo-env.d.ts | infra | P3 | `[ok 3ee3bb2]` symlink evita TS2882 falso em worktree (achado confirmado por 4 agentes) |
| R-BUNDLE-DIET-CALENDARS-REPLACE | refactor | P3 | `[todo]` refactor grande (3-5d): substituir react-native-calendars por custom. Spec materializada |
...
| R-OPS-4 | Branch protection | infra | — | doc+script prontos | `[ok-doc]` doc canonica em docs/RELEASE.md:382-459 + scripts/setup-branch-protection.sh idempotente (escrita em 2026-05-17). Aplicacao via API depende dono rodar `./scripts/setup-branch-protection.sh` (diff atual: contexts falta "Build APK Android", required_linear_history=false, allow_force_pushes=true) |
...
| `[ok]` | M-GAUNTLET-DEAD-CODE-V2 | Refactor: `gauntletBootstrap.ts` + `gauntletAtivo.ts` + `gauntletDashboard.tsx` com `require` lazy. `app/_layout.tsx`, `adicionarFotoManual.ts`, `useFotosAgregadas.ts` migrados para `MODO_DEV_WEB`. Identificadores renomeados para nomes neutros (`iniciarModoDev`, `sinalizarBootDev`, etc) — refinamento defensivo. Leak check 0/6 markers no bundle Hermes 9,8 MB. Fechada na verdade em 2026-05-04 (commit `b6419b4`); re-validada em 2026-05-21 (worktree `7d0702f` só atualiza header do spec) — falha de auditoria pré-dispatch consumiu agente; bug operacional registrado | — | — | 4-6h | `M-GAUNTLET-DEAD-CODE-V2-spec.md` |
