...
| `[todo]` | M24.1 | Resume state — `setUltimaRota('/memoria') + reload` abre app em `/`. Hipótese: race entre hidratação assíncrona da `useSessao` e o `Redirect` do `SessaoBootGate`. Bloqueia M29 | — | — | 1-2h | `M24.1-spec.md` |
| `[todo]` | M25.2 | Animação Reanimated não atualiza `transform` de SVG em web. Fix M25.1 (string) aplicado mas atributo permanece `rotate(0 160 160)` em 3 amostras consecutivas. Trocar por `Animated.View` style transform ou writeback via ref. Bloqueia M29 | — | — | 2-3h | `M25.2-spec.md` |
| `[todo]` | M27.2 | Boot screen oscilante (regressão de M27.1). Loader Ouroboros volta a sobrepor após conteúdo da rota carregar. Fix M27.1 trata fontes mas não o gate de sessão. Bloqueia M29 | — | — | 2-4h | `M27.2-spec.md` |
