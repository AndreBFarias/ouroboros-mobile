...
| `[wip]` | M25.1 | Fix animação OuroborosLoader em web. Substitui `useAnimatedProps` `{rotation, originX, originY}` por string `transform="rotate(N 160 160)"` (RN-SVG-web não converte origin null; rotação acontecia em torno de (0,0)). Detectado em validação manual 2026-05-03 | — | — | 30min | `M25.1-spec.md` |
| `[wip]` | M27.1 | Fix boot screen lento (~10s reload) e overlay sobreposto à Home. Investigar `useFonts` oscilante em SDK 54 + BiometriaGate timing + useConquistas em web. Adicionar `useRef` guard ou fade transition no `_layout.tsx`. Detectado em validação manual 2026-05-03 | — | — | 1-2h | `M27.1-spec.md` |
