...
USO: Este arquivo + CONTEXTO.md alimentam a ferramenta de automacao de
     desktop para desenvolver o App Android numa pasta separada de
     protocolo-ouroboros.
...
tempo-alvo em segundos. Design system fechado com hex codes, detalhado
neste documento (Seção 2) e materializado nos tokens de
`src/theme/tokens.ts`.
...
ADR-006 (`docs/ADRs/0006-stack-expo-rn.md`) detalha a decisão. Resumo dos motivos:
...
- **Ecossistema gigantesco** = documentação e material de referência abundantes
...
## 12. Fonte de Verdade Visual

Paleta, tipografia, spacing e princípios estéticos deste documento
(Seção 2) foram materializados em `src/theme/tokens.ts`, que é a
referência de tokens em runtime. A validação visual de cada tela é feita
em runtime no device a cada sprint.
...
- `docs/ADRs/` — Architecture Decision Records: stack, sync, vault,
  layout, bundle e demais decisões formalizadas
