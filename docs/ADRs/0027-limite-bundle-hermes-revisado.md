...
- **Limite documentado oficialmente** elimina ambiguidade entre
  ...
  `8,55 MB` / `8,85 MB` como teto. O gate de validação passa a citar
...
documentação canônica. O gate de validação continuaria recusando M41
...
Documentos relacionados que serão atualizados posteriormente (fora
desta sprint):
