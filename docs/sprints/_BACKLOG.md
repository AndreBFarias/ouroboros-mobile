...
| R-RECAP-PERIODO-DIA | Adicionar período "dia" ao Recap (achado R-HOME-1, decisao pendente do dono) | feature | P3 | 1-2h | `R-RECAP-PERIODO-DIA-spec.md` |
| R-INFRA-GAUNTLET-WORKTREE-SYMLINK `[ok]` (DX.4) | Gauntlet web em worktree não carrega rotas locais | infra+DX | P2 | 2-3h | `R-INFRA-GAUNTLET-WORKTREE-SYMLINK-spec.md` |
...
| R-INT-3-HC-PROXY-REFLECT-HARDENING `[ok]` (DX.1, `5d41ca6`) | Detectar Proxy lancante de react-native-health-connect 3.5.0 via Reflect.get | hardening+DX | P2 | 1h | `R-INT-3-HC-PROXY-REFLECT-HARDENING-spec.md` |
...
## Fila de cobertura — módulos de alto risco sem teste direto

> Semeada por **AUDIT-P3-7** (2026-09-05). A sprint cobriu apenas
> `src/lib/stores/persist.ts` (100% nas quatro colunas) e fixou o piso
> global em `jest.config.js` (74 / 63 / 74 / 76). Os módulos abaixo
> ficam explícitos aqui para não se perderem — cada um é sprint
> própria, e vários exigem decisão de estratégia antes (RTL de tela,
> E2E, ou unitário sobre lógica extraída).
>
> Todos os caminhos foram conferidos como existentes em 2026-09-05.

| # | Módulo | Refs em `tests/` | Por que importa | Estratégia provável |
|---|---|---|---|---|
| 1 | `app/settings/vault.tsx` | 0 | tela de export/wipe/reset do Vault. Existe E2E de export (`r-vault-b-settings-export.e2e.ts`), mas wipe e reset — os destrutivos — não aparecem cobertos | RTL de tela, ou extrair as ações destrutivas para módulo testável |
| 2 | `src/lib/schemas/financeiro.ts` | 0 | valida notificação financeira vinda de **outro app** via share intent Android — entrada não confiável por natureza. Cuidado com falso positivo: os matches de `schemas/financeiro` em `tests/` são de `schemas/financeiro_nota`, arquivo diferente | unitário de schema, com fixtures de entrada malformada |
| 3 | `app/share-receive.tsx` | 0 | a tela que recebe essa entrada não confiável. O E2E existente (`m-q10-share-financeiro.e2e.ts`) declara no cabeçalho que exige checkpoint humano — não é prova automatizada | RTL de tela sobre o receiver |
| 4 | `src/lib/scanner/launch.ts` | 0 | pipeline do Scanner. `parsing.ts` e `saveNota.ts` já têm teste; este não | unitário com a fronteira nativa mockada |
| 5 | `src/lib/scanner/multipage-pdf.ts` | 0 | idem | unitário |
| 6 | `src/lib/scanner/text-recognition.ts` | 0 | idem. O E2E existente documenta que o fluxo real usa ML Kit e câmera e não roda no Gauntlet web — cobre navegação, não parsing | unitário com ML Kit mockado |
| 7 | `app/ciclo/registrar.tsx` | 0 | escreve dado de saúde sensível. Mitigado parcialmente por 2 E2E de persistência, sem teste isolado de validação de formulário | RTL de tela |
| 8 | `src/lib/diario/transcribe.ts` | 0 | transcrição de áudio do diário. Fronteira nativa e assíncrona, consumida por 3 componentes de UI. Zero cobertura em qualquer nível | unitário com a fronteira nativa mockada |
| 9 | `src/lib/stores/hydrated.ts` | 0 | gate de "hidratação pronta" no boot. Bug aqui = corrida de boot, tela em branco ou render antes do Vault carregar | unitário de store |

Duas filas correlatas, também não desta sprint:

- **28 arquivos de `src/lib` sem teste direto** no total (11,1% de 252) —
  a maioria de baixo risco, mas a lista está medida.
- **Branch coverage sistematicamente atrás de statements** (63,4% contra
  74,5% na medição de 2026-09-05): os caminhos de erro são menos
  testados que os felizes. Consistente com o achado paralelo de 6 testes
  cuja única asserção é `not.toBeNull()` / `toBeDefined()` sobre o caso
  positivo, sem verificar conteúdo.

> **Nota de versionamento**: este arquivo está *untracked* e o
> repositório é público. A fila acima é material interno de processo.
> Se o dono quiser versioná-la, isso precisa de spec própria — e nunca
> por `git add docs/sprints/`, sempre lista explícita de arquivos.
