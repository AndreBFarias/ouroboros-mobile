...
// R-RECAP-8 (2026-07-10): log de diagnostico dev-only do motivo da
// falha de captura. Some do bundle release (dead-code via __DEV__).
...
export async function exportarSlideMemorias(
  ...
  try {
    ...
    // nativo (ex: NullPointerException de child null em arvore
    // Reanimated no Fabric -- A46; mitigado por capturar o card
    // estatico ShareCardMemoria). Log dev-only para diagnostico no
    // logcat; o caller exibe toast "Nao foi possivel gerar a imagem.".
    devLog('[recap-share] captura falhou', {
      slideId: opcoes.slideId,
      formato,
