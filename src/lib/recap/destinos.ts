...
// R-HOME-4d: destino do card "Relembrando" da home viva. Centraliza a
// navegacao das tres origens do pool de lembrancas:
//   reflexao/conquista -> /diario-emocional?slug=<id> (mesmo destino de
//     destinoReflexao/destinoConquista origem diario_vitoria; o slug e
//     ignorado hoje pela tela, limitacao pre-existente compartilhada
//     com o Recap).
//   humor -> /humor (historico de humor; humor nao tem detalhe por item).
export function destinoRelembranca(r: {
  origem: 'reflexao' | 'humor' | 'conquista';
  id: string;
}): DestinoRecap | null {
  switch (r.origem) {
    case 'reflexao':
    case 'conquista':
      return { pathname: '/diario-emocional', params: { slug: r.id } };
    case 'humor':
