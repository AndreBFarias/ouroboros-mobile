...
export function MicrofoneButton({ onAudioGravado }: MicrofoneButtonProps) {
  ...
  // que ficou no cache (lição M06.5 revisao -- usuario
  ...
  // precisa rodar mesmo apos unmount (lição M06.5).
  const finalizar = async (gracioso: boolean) => {
    ...
    if (!gracioso || !vaultRoot) {
      ...
      // antes de devolver controle a UI (lição M06.5 revisao
      // -- evita acumulo em sessoes longas).
